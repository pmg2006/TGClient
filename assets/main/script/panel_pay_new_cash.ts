...
export default class panel_pay_new_cash extends cc.Component {
    ...
    //印度代付必填：收款人银行账号，收款人姓名，印度IMPS必填IFSC码，印度银行必填银行编码，印度填手机号
    @property(cc.EditBox)
    bankAccount: cc.EditBox = null;//银行账号

    @property(cc.EditBox)
    userName: cc.EditBox = null;//银行密码

    @property(cc.EditBox)
    bankCode: cc.EditBox = null;//银行编码

    @property(cc.EditBox)
    phone: cc.EditBox = null;//印度手机号

    @property(cc.EditBox)
    imps: cc.EditBox = null;//IMPS
    ...
    bindObj = [];
    start() {
        this.totalBonus.string = "₹" + KernelData.score.toFixed(2);
        this.cashBonus.string = "₹" + Math.min(KernelData.score, KernelData.withdrawAbleScore).toFixed(2);
        ...
        this.bankAccount.string = KernelData.bankaccount?.bankAccount || "";
        this.userName.string = KernelData.bankaccount?.userName || "";
        this.bankCode.string = KernelData.bankaccount?.bankCode || "";
        this.phone.string = KernelData.bankaccount?.phone || KernelData.mobile || "";
        this.imps.string = KernelData.bankaccount?.cpf || "";
    ...
    btnOnClick(event) {
        ...
        let pixCPF = this.imps.string;//这里是印度的IMPS的IFSC码
        let username = this.userName.string;
        let bankAccount = this.bankAccount.string;
        //let bankCode = this.bankCode.string;
        ...
        userInfo.bankCode = bankAccount;//bankCode改为银行账号
        ...
        // console.log("提现请求--", url, JSON.stringify(msg));
        // return;
