...
export default class Hall extends cc.Component {
    ...
    gameData = {
        ...
        4: GameData.Instance.liveCasinoUrls,
        5: GameData.Instance.blockchainUrls,
        6: GameData.Instance.arcadeUrls,
    };
        ...
        if (KernelData.userID && this.curPageIndex == EM_PAGEVIEW_TYPE.LocalGame) {
            ...
            this.game_pageView.scheduleOnce(() => {
                cc.director.once(cc.Director.EVENT_AFTER_DRAW, () => {
                    ...
                    this.hall_scrollview.content.height = this.hall_scrollview.content.height + this.searchNode.height + (this.nodeDownload.height / 2);
                    //} else {
    ...
    start() {
        ...
        GameData.Instance.deviceInfo = os;
        ...
        for (let pageIndex of ["1", "2", '3', '4', '5', '6']) {
            ...
            let gameListNode = page_1_Node.getChildByName(`Game${pageIndex}`).getChildByName("gameList");
            {
                ...
                let lable = page_1_Node.getChildByName(`Game${pageIndex}`).getChildByName("Head").getChildByName("button").getChildByName("Background").getChildByName("Label").getComponent(cc.Label);
                lable.string = 'View ' + gameData[pageIndex].length + ' Games';
            }
        ...
        if (this.searchNode.active) {
            this.hall_scrollview.content.height = this.hall_scrollview.content.height + contentHeight + this.searchNode.height + GameData.Instance.JackpotHeight + (this.nodeDownload.height / 2);
        } else {
            this.hall_scrollview.content.height = this.hall_scrollview.content.height + contentHeight + GameData.Instance.JackpotHeight + (this.nodeDownload.height / 2);
        }
    ...
    onChooseGameType(event) {
        ...
        this.game_pageView.scheduleOnce(() => {
            cc.director.once(cc.Director.EVENT_AFTER_DRAW, () => {
                ...
                this.hall_scrollview.content.height = this.oldScrollContentHeight + pageIndexHeight + 100 + (this.nodeDownload.height / 2);
            }, this);
        }, 0);
    ...
    loadMoreItems() {
        ...
        this.game_pageView.scheduleOnce(() => {
            cc.director.once(cc.Director.EVENT_AFTER_DRAW, () => {
                let pageIndexHeight = this.game_pageView.node.getChildByName("view").getChildByName("content").getChildByName(`page_${pageIndex}`).height;
                if (this.searchNode.active) {
                    this.hall_scrollview.content.height = this.oldScrollContentHeight + pageIndexHeight + this.searchNode.height + GameData.Instance.JackpotHeight + (this.nodeDownload.height / 2);
                } else {
                    this.hall_scrollview.content.height = this.oldScrollContentHeight + pageIndexHeight + GameData.Instance.JackpotHeight + (this.nodeDownload.height / 2);
                }
            }, this);
        }, 0);
    }
    ...
    //by_009:按钮点击事件 - 请求游戏链接
    async onBtnClick(event) {
        // // 检查用户余额
        // if (KernelData.score <= 0) {
        //     UIHelper.showTips("Insufficient balance");
        //     return;
        // }
        ...
        if (event.currentTarget.name.indexOf("pg") != -1
            || event.currentTarget.name.indexOf("bg") != -1
            || event.currentTarget.name.indexOf("jili") != -1
            || event.currentTarget.name.indexOf("cq9") != -1
            || event.currentTarget.name.indexOf("jdb") != -1
            || event.currentTarget.name.indexOf("spribe") != -1) {
            ...
            console.log("当前游戏类型:", GameData.Instance.gameType);
            console.log("当前游戏名称:", gameName);
            //如果pg游戏 且 游戏类型为vintebet
            if (event.currentTarget.name.indexOf("pg") != -1 && GameData.Instance.gameType == GAME_TYPE.VINTEBET) {
                // 提取游戏编号，例如从"pgsoft_126"中提取出"126"
                let pureGameCode = gameName.replace("pg_", "");
                ...
                //} else if (event.currentTarget.name.indexOf("pg") != -1 && GameData.Instance.gameType == GAME_TYPE.EGAME) {
            } else if (GameData.Instance.gameType == GAME_TYPE.EGAME) {
                // 提取游戏编号，例如从"pgsoft_126"中提取出"126"
                //let pureGameCode = gameName.replace("pg_", "");

                //let gameCode = GameData.Instance.getNewPgGameCode(pureGameCode);
                //let gameCode = pureGameCode;
                let pureGameCode = gameName;
                let gameCode = gameName;
                console.log("提取的游戏编号为:", gameCode, " 原来的编号为:", pureGameCode);
                UIHelper.showWaitLayer(true);
            ...
            case "btn_dw"://APP下载页面
                {
                    ...
                    url = "https://luck111.s3.ap-south-1.amazonaws.com/GoodLuck.apk";
                    // }else {
