...
import { resLoader, ProcessCallback } from "./ResLoader";
//import ModelManager from "../model/ModelManager";
//import { UICF } from "../example/UIExample";
...
export class UIManager {
    ...
    private autoExecAnimation(uiView: UIView, aniName: string, aniOverCallback: () => void) {
        if (uiView.popType == AniType.Null) {
            aniOverCallback()
            return
        }
        if (aniName == 'onClose') {
            if (uiView.popType == AniType.Scale) {
                uiView.node.stopAllActions()
                let act = cc.scaleTo(0.3, 0).easing(cc.easeBackIn())
                act = cc.sequence(act, cc.callFunc(aniOverCallback))
                uiView.node.runAction(act)
            }
        } else {
            if (uiView.popType == AniType.Scale) {
                uiView.node.scale = 0
                ...
                let scale = 1
                if (size.width / size.height < 0.5625) {
                } else {
                    //宽度适配
                    scale = size.height * 0.5625 / size.width
                }
                let act = cc.scaleTo(0.3, scale).easing(cc.easeBackOut())
                act = cc.sequence(act, cc.callFunc(aniOverCallback))
                uiView.node.runAction(act)
            }
        }
    ...
    private updateUI() {
        ...
        for (; showIndex >= 0; --showIndex) {
            ...
            if (this.UIStack[showIndex] && this.UIStack[showIndex].uiView && this.UIStack[showIndex].uiView.node) {
                this.UIStack[showIndex].uiView.node.active = true;
            } else {
                console.log(`updateUI show ${showIndex} faile!`);
            ...
            if (UIShowTypes.UIFullScreen == mode) {
                ...
                for (let i = 0; i < this.BackGroundUI; ++i) {
                    if (this.UIStack[i] && this.UIStack[i].uiView && this.UIStack[i].uiView.node) {
                        this.UIStack[i].uiView.node.active = true;
                    } else {
                        console.log(`updateUI UISingle ${i} faile!`);
                    }
                }
        ...
        for (let hide: number = hideIndex; hide < showIndex; ++hide) {
            if (this.UIStack[hide] && this.UIStack[hide].uiView && this.UIStack[hide].uiView.node) {
                this.UIStack[hide].uiView.node.active = false;
            } else {
                console.log(`updateUI hide ${hide} faile!`);
            }
        }
    }
     ...
     */
    private async onUIOpen(uiId: number, uiView: UIView, uiInfo: UIInfo, uiArgs: any) {
        if (null == uiView) {
            return;
        }
        // 激活界面
        ...
        let child = cc.find('Canvas').getChildByName('UI');
        this.viewIndex++

        uiView.node.zIndex = this.viewIndex
        if (uiView.popType == AniType.Scale)
            uiView.node.zIndex = this.viewIndex
        // 快速关闭界面的设置，绑定界面中的background，实现快速关闭
        if (uiView.blackBg) {
            uiView.background = cc.instantiate(this.background)
            uiView.background.active = true
            uiView.background.zIndex = uiView.node.zIndex + 1
            uiView.node.zIndex += 2
            uiView.background.opacity = 150
            uiView.background.parent = child
            uiView.background.on(cc.Node.EventType.TOUCH_START, (event: cc.Event.EventCustom) => {
                if (uiView.quickClose) {
                    uiView.background.active = false
                    uiView.background.stopAllActions()
                    event.stopPropagation();
                    this.close(uiView);
                }
            }, this);
        ...
        this.autoExecAnimation(uiView, 'onOpen', () => {
    ...
    /** 打开界面并添加到界面栈中 */
    public open(uiId: number, uiArgs: any = null, progressCallback: ProcessCallback = null, completeCallback = null): void {
        let startTime = (new Date).getMilliseconds();
        if (this.netLoading == null) {
            this.netLoading = cc.find('Canvas/loading')
        }
        if (!this.netLoading.active) {
            this.netLoading.active = true

            let act = cc.sequence(cc.delayTime(30), cc.callFunc(() => {
                this.netLoading.active = false
            }))
            this.netLoading.runAction(act)
            //this.netLoading.runAction(act);
            let label = this.netLoading.getChildByName("loading_Lab").getComponent(cc.Label);
            label.string = "资源加载中...";
        }
        ...
        this.getOrCreateUI(uiId, progressCallback, (uiView: UIView): void => {
            // 如果界面已经被关闭或创建失败
            this.netLoading.stopAllActions();
            this.netLoading.active = false;
            if (uiInfo.isClose || null == uiView) {
                let l = `getOrCreateUI ${uiId} faile! close state : ${uiInfo.isClose} , uiView : ${uiView}`;
                cc.log(l);
                let obj = { msg: l}
                //ModelManager.getInstance().roleModel.sendDebugInfo(JSON.stringify(obj));
            ...
            if (cost>= 10 * 1000) {
                let obj = { msg: `加载超时UIID: ${uiId}时间超过 ${cost/1000} 秒`}
                //ModelManager.getInstance().roleModel.sendDebugInfo(JSON.stringify(obj));
            }
        }, uiArgs);
    }
    ...
    /** 替换栈顶界面 */
    // replace会替换掉当前的界面，播放uiId界面的打开动画，关闭当前界面
    public replace(uiId: number, uiArgs: any = null, complete = null) {
        let closeitem : UIView = null; 
        let uiInfo: UIInfo = null;
        if (this.UIStack.length) {
            uiInfo = this.UIStack[this.UIStack.length - 1];
            closeitem = uiInfo.uiView;
        }
        this.open(uiId, uiArgs, null, () => {
            if (closeitem) {
                //console.log("replace close uiview: " + closeitem.name + " open uiId: " + UICF[uiId].prefab);
    ...
    public close(closeUI?: UIView) {
        /** */
        if (closeUI.background) {
            closeUI.background.destroy()
            closeUI.background = null
        }
        ...
        let close = () => {
            ...
            // 显示之前的界面
            if (preUIInfo && preUIInfo.uiView && preUIInfo.uiView.node && this.isTopUI(preUIInfo.uiId)) {
            ...
            if (uiView.cache) {
                ...
                console.log(`uiView is cache, removeFromParent ${uiInfo.uiId} name:${uiView.name}`);
            } else {
                //uiView.releaseAutoRes();
                uiView.node.destroy();
                console.log(`uiView is not cache, destroy ${uiInfo.uiId} name:${uiView.name}`);
            }
            this.autoExecNextUI();
        }
        // 执行关闭动画
        this.autoExecAnimation(uiView, 'onClose', close);
    }
    ...
    public closeAll() {
        ...
        for (const uiInfo of this.UIStack) {
            ...
            if (uiInfo.uiView) {
                ...
                console.log("关闭界面：" + uiInfo.uiId + " name:" + uiInfo.uiView.name);
