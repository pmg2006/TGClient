const SubgameManager = {
    ...
    _getfiles: function(name, type, downloadCallback, finishCallback) {
        ...
        /// 替换该地址
        var UIRLFILE = "http://dwc_test.xuankai6.com:8080/native/subgame/" + name;
        ...
        if (type === 1) {
            ...
            this._am.setEventCallback(this._needUpdate.bind(this, name));
        }
    ...
    _updateCb: function(event) {
        ...
        switch (event.getEventCode()) {
            ...
            case jsb.EventAssetsManager.UPDATE_PROGRESSION:
                /*5 最新进展 */
                self._downloadCallback && self._downloadCallback(event.getPercentByFile(), self._fileName);
                break;
            ...
            case jsb.EventAssetsManager.UPDATE_FINISHED:
                /*8更新完成*/
                self._finishCallback && self._finishCallback(true, self._fileName);
                break;
        ...
        if (failed) {
            ...
            self._updating = false;
            self._finishCallback && self._finishCallback(false, self._fileName);
        }
    },
    ...
    // type = 3
    _needUpdate: function(name, event) {
        let self = this;
        switch (event.getEventCode()) {
            case jsb.EventAssetsManager.ALREADY_UP_TO_DATE:
                console.log('子游戏已经是最新的，不需要更新' + name);
                self._finishCallback && self._finishCallback(false, name);
                break;

            case jsb.EventAssetsManager.NEW_VERSION_FOUND:
                console.log('子游戏需要更新' + name);
                self._finishCallback && self._finishCallback(true, name);
                break;
            ...
            case jsb.EventAssetsManager.ERROR_UPDATING:
            case jsb.EventAssetsManager.UPDATE_FAILED:
                console.log('更新有问题' + name);
                self._downloadCallback(0, name);
                break;
            default:
                break;
